...
int pion_capturer(int mat[N][N],int joueur,int x_donne,int y_donne){
	...
	int autre_joueur;
	int possible=0;	//possible à capturer
		...
		//	printf("d");
			...
			if(mat[i][j]==joueur&&mat[i-1][j+1]==autre_joueur){	//condition à droite haut
				x=i-1;
				y=j+1;				
				while(x>=0&&y<N){
					if(mat[x][y]==joueur){
			...
			if(mat[i][j]==joueur&&mat[i+1][j+1]==autre_joueur){	//condition à droite bas
				x=i+1;
				y=j+1;				
				while(x<N&&y<N){
					if(mat[x][y]==joueur){
			...
			if(mat[i][j]==joueur&&mat[i-1][j-1]==autre_joueur){	//condition à gauche haut
				x=i-1;
				y=j-1;				
				while(x>=0&&y>=0){
					if(mat[x][y]==joueur){
			...
			if(mat[i][j]==joueur&&mat[i+1][j-1]==autre_joueur){	//condition à gauche bas
				x=i+1;
				y=j-1;				
				while(x<N&&y>=0){
					if(mat[x][y]==joueur){
			...
			if(possible){		//à droite gauche bas
			//printf("e");
				x=i+1;
				y=j-1;	
				while(x<N){				
